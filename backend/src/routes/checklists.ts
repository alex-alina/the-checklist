...
// const findChecklist = async (checklistId: string) => {
//   const repository = AppDataSource.getRepository(Checklist);
//   return repository.findOne({ where: { id: checklistId } });
// };
...
//     const checklist = await findChecklist(checklistId);
//     if (!checklist) {
...
//       return res.status(400).json({ message: 'Checklist and item IDs are required' });
//     }

//     const repository = AppDataSource.getRepository(Item);
//     const item = await repository.findOne({ where: { id: itemId }, relations: ['checklist'] });
